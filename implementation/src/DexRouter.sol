...
import {IActivityMonitor} from "./recycle/interfaces/IActivityMonitor.sol";
...
contract DexRouter is IDexRouter {
    ...
    address public activityMonitor;
    ...
    function setActivityMonitor(address monitor) external onlyOwner {
    ...
    function _recordActivity(address token, uint256 volumeNative) internal {
        address monitor = activityMonitor;
        if (monitor != address(0) && volumeNative > 0) {
            IActivityMonitor(monitor).recordSwap(token, volumeNative);
        }
    }
        ...
        ensure(deadline)
    {
        ...
        _recordActivity(token, amountIn);
    }
    ...
    function sell(uint256 amountIn, address token, address to, uint256 deadline) external ensure(deadline) {
        ...
        _recordActivity(token, amountOut);
    }
