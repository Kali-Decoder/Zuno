...
import {ILPRecyclingVault} from "./recycle/interfaces/ILPRecyclingVault.sol";
...
contract BondingCurve is IBondingCurve {
     ...
     * @dev Creates trading pair, provides initial liquidity, locks LP in recycling vault
     */
    function listing() external returns (address) {
        ...
        require(realNativeReserves >= listingFee, "LISTING_FEE");

        // Burn excess tokens so LP matches current virtual price; skip if already below target ratio
        {
            uint256 tokensForLp = ((realNativeReserves - listingFee) * virtualToken) / virtualNative;
            if (tokensForLp < realTokenReserves) {
                uint256 burnTokenAmount = realTokenReserves - tokensForLp;
        ...
        address vault = _factory.getLpVault();
        require(vault != address(0), "LP_VAULT_NOT_SET");
        // Lock LP in recycling vault instead of permanent burn
        IUniswapV2ERC20(pair).transfer(vault, liquidity);
        ILPRecyclingVault(vault).registerLock(token, pair, liquidity, msg.sender);
