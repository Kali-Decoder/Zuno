...
import { getFreshPublicProvider, waitForPublicBlock } from "@/app/lib/provider";
import { txError } from "@/app/lib/format";
...
export function decodeCallError(error: unknown, method = "transaction") {
  ...
  if (raw.toLowerCase().includes("missing revert data")) {
    return `${method} needs more gas than the wallet estimated. Retry — the app sets gas from the Monad RPC. Stay on Monad Testnet and keep some MON for gas.`;
  }
  return raw;
}
