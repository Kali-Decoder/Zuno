...
const AirdropProbability = () => {
  ...
  const isConnected = authenticated && !!accountAddress;
  const { data, loading: isLoading } = useGetAccount(accountAddress || "");
  if (!isConnected || !ready || !authenticated) return null;
  return (
    <div className="flex items-center gap-[0.6rem] sm:gap-[0.8rem] bg-white/[0.02] hover:bg-white/[0.04] border border-white/[0.08] hover:border-white/[0.12] rounded-xl px-[1rem] sm:px-[1.4rem] py-[0.6rem] sm:py-[0.8rem] transition-all duration-200 backdrop-blur-sm shadow-lg">
