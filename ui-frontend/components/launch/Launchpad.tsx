...
const PRESET_IMAGES = [
  { src: "/coins/Chamaleon.png", label: "Chameleon" },
  { src: "/coins/SunPixel.png", label: "SunPixel" },
  { src: "/coins/AQ.png", label: "Astro" },
  { src: "/gmonad.jpeg", label: "Reflow" },
  { src: "/diamondHands.png", label: "Diamond" },
  { src: "/gmonad.jpeg", label: "Gmonad" },
];
...
export default function Launchpad() {
  ...
  const launch = async () => {
    ...
    if (!wallet.isCorrectNetwork) {
      toast.error("Switch to Monad Testnet");
    ...
    try {
      ...
      const ok = await wallet.runWrite(async signer => {
        const result = await createTokenCurve({
          ...
          tokenURI: form.imageUrl || "ipfs://reflow",
      ...
      if (ok && resultAddress) {
        ...
        toast.success("Token launched on Monad Testnet");
      }
    } catch (error) {
        ...
        <div className="tabs-wrapper max-w-[72rem] mx-auto items-center text-center gap-[2.4rem] py-[4rem]">
          ...
          <div className="space-y-[0.8rem]">
            ...
            <p className="text-white/60 text-[1.2rem] sm:text-[1.4rem] max-w-[42rem] mx-auto">
              Your bonding curve is live on Monad Testnet. Share the token and invite early buyers.
            </p>
          </div>
          <div className="relative size-[12rem] rounded-md overflow-hidden border border-white/10 bg-white/5">
            <Image src={launched.imageUrl} alt={launched.name} fill className="object-cover" />
          </div>
    ...
    <div className="page-container pb-[6rem]">
      <div className="mb-[2.4rem] sm:mb-[3.2rem] space-y-[1.2rem]">
        ...
        <p className="text-white/60 text-[1.2rem] sm:text-[1.5rem] max-w-[56rem]">
          Create a token, seed the curve, and graduate later. Launches write to Core.createCurve on Monad Testnet.
        </p>
      </div>
            ...
            <div className="space-y-[2rem]">
              <div className="flex flex-col sm:flex-row gap-[1.6rem] items-start">
                ...
                <div className="space-y-[0.6rem]">
                  ...
                  <p className="font-mono text-[1.2rem] text-white/80">Seed buy: {form.seedBuy || "0"} MON</p>
                </div>
              </div>
